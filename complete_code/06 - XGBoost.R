...
param <- list(objective = "binary:logistic",  booster = "gbtree",colsample_bytree = 0.7,
                eval_metric = "auc", eta = 0.02, max_depth = 5, subsample = 0.7)

#Creamos una función para entrenar los modelos
xgb_model <- function(xgb_df, nr, plot_xgb=TRUE){
  modelo <- xgb.train(params = param, data  = xgb_df, nrounds = nr, verbose = 1, maximize = FALSE)
